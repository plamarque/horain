 ...
 *   - LANGSMITH_DATASET_ID — required unless --create-dataset is used
...
async function createDataset(base, apiKey, name, description) {
  const body = JSON.stringify({
    name,
    description: description || 'Imported from Horain Promptfoo corpus (langsmith/scripts/import-promptfoo-to-langsmith.mjs)'
  })
...
async function main() {
  ...
  if (!apiKey) {
    console.error('Missing LANGCHAIN_API_KEY (set in environment or backend/.env; use --no-backend-env to skip loading backend/.env)')
    process.exit(1)
  }

  let datasetId = process.env.LANGSMITH_DATASET_ID?.trim()
  if (!datasetId && !options.createDataset) {
    console.error(
      'Missing LANGSMITH_DATASET_ID. Set it to an existing dataset UUID, or pass --create-dataset to create one explicitly.'
    )
    process.exit(1)
  }
  ...
  if (options.dryRun) {
    ...
    if (!datasetId) {
      ...
      console.error(`Dry-run: would create dataset: ${name}`)
    } else {
      console.error(`Dry-run: would upload to dataset: ${datasetId}`)
    }
    process.exit(0)
  }
  ...
  if (!datasetId) {
    ...
    datasetId = await createDataset(base, apiKey, name)
  ...
  for (let i = 0; i < examples.length; i++) {
    ...
    const idx = i + 1
    await postExample(row, datasetId, base, apiKey, idx)
    ok++
