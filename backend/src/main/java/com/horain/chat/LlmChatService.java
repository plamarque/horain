...
public class LlmChatService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            - MASS UPDATE GUARD: When the user asks to apply a change to "all" or "toutes" activities for a project (e.g. set all to billable, toggle billable for every entry of a project), you MUST first fetch the entries (using the all-time range above if no period was specified), then tell the user how many entries will be affected and ask for explicit confirmation (e.g. "You are about to set N entries to billable. Confirm? (yes / go ahead)"). Do NOT call update_time_log in a loop until the user has confirmed.
