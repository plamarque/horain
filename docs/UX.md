...
- **Agent trace (reasoning visibility):** Just below each assistant message bubble (before any chart or activity cards), a small **trace** block shows which tools were used for that turn. During the response it shows a minimal “Thinking...” indicator; once the response is complete, a collapsible line “Outils utilisés : &lt;type labels&gt; (e.g. “Exploration, Lecture, Écriture”)” appears (collapsed by default). When expanded, each **turn** (iteration) is a collapsible block with a one-line summary (e.g. “Lecture · 2 outils, Exploration · 1 outil”); when a turn is expanded, tool calls are grouped by **task type** (Exploration, Lecture, Écriture, Suppression, Autre) with accordion sections. **Primary display is in natural language** (e.g. “Chargement des temps pour graphique agrégé”, “Enregistrement d’un temps”) so the user sees what the agent is doing; the raw tool name is shown in smaller text for debugging. **Discreet presentation (Cursor-style):** the trace is not framed (no box or strong border), uses attenuated/muted colors, and the expanded detail scrolls within a limited height (~14rem) before collapsing. Only the current turn stays expanded during streaming; when the next turn starts, the previous turn collapses to its summary. After the stream ends, the last turn stays expanded. The trace is **session-only** (not persisted). An optional **Thinking** section (model reasoning text) is shown when the backend exposes it: the stream sends `reasoning_chunk` events and the `done` payload may include `reasoningText` and `reasoningDurationMs`. The UI displays a collapsible “Thought for Xs” block (Cursor-style): the **full reasoning** in muted grey (expand/collapse), and **below it a one-line summary** in white (or high-contrast) so the user gets the gist without opening the block. The summary is either sent by the backend as `reasoningSummary` or derived client-side (first sentence or first ~120 characters of `reasoningText`). It appears only when the model provides reasoning (e.g. with a reasoning-capable model and Responses API).
  ...
  **Référence UX type Cursor (séquences à répliquer) :**
  ...
  | Tour en cours (outils en direct) | Tool calls qui s’ajoutent au fur et à mesure (e.g. “Read AgentTraceBlock.vue L1-100”, “Explored 2 files”) | Tool calls streamés, groupés par type (Lecture, Exploration, …) dans le tour courant |
  | Fin du tour | En-tête résumé du tour (e.g. “Thought for 35s”) | Résumé une ligne par tour (e.g. “Lecture · 2 outils, Exploration · 1 outil”) |
  | Passage au tour suivant | Le bloc du tour précédent se replie : seul l’en-tête reste visible | Tour précédent replié (résumé visible, détail masqué) ; tour courant ouvert |
  | Tour replié (clic pour rouvrir) | Clic sur “Thought for Xs” → raisonnement + tool calls détaillés | Clic sur “Thought for Xs” → texte de raisonnement ; clic sur le résumé du tour → sections par type + détail des appels |
  | Phases / types de tâche | “Exploring”, “Read”, etc. | Exploration, Lecture, Écriture, Suppression, Autre (voir agentTraceTaskTypes) |

  Autres patterns Cursor utiles (doc / blog) : **Debug Mode** (hypothèses → instrumentation → reproduction → analyse logs → correctif ciblé) et **Plan Mode** (plan en markdown avant exécution) mettent l’accent sur la visibilité des étapes et la vérification humaine ; pour la trace in-chat, on s’aligne sur les séquences ci-dessus (Thinking → résumé par tour → collapse au tour suivant).
