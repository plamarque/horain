...
- **Activity cards layout:** Log entries are shown as **flip cards** below the message bubble (mobile-friendly). **Recto:** date, duration, and the start of the note (up to 2–3 lines with ellipsis), with a background colour derived from the project. **Verso:** project name, dollar icon ($) if the entry is billable, and the note. Tap/click flips the card and adds the entry to context when applicable. **Right-click** (desktop) or **long-press** (mobile) on a card opens a context menu with "Edit entry" and "Edit project". **Double-click on the project name** (on the verso) opens the **project** edit modal (name, description, Facturable). **Double-click on the card** opens the **entry** edit modal (duration, note, Facturable, date). A “+N more” button expands the list when there are many entries.
