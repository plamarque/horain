...
- **Agent trace (reasoning visibility):** Just below each assistant message bubble (before any chart or activity cards), a small **trace** block shows which tools were used for that turn. During the response it shows a minimal “Réflexion…” indicator; once the response is complete, a collapsible line “Outils utilisés : &lt;tool names&gt;” appears (collapsed by default). The user can expand it to see, per tool: name, success/error status, arguments (truncated), and result (truncated). When the backend sends iteration data, tool calls are grouped by “Tour 1”, “Tour 2”, etc. The trace is **session-only** (not persisted); it remains readable for the duration of the session so the user can verify what the agent did and restore confidence in its actions.
