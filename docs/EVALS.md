...
**Plateforme externe (optionnel) :** l’export vers LangSmith (runs, feedback, script dataset) est **optionnel** et ne remplace pas Promptfoo ni les tables `agent_turn` / `agent_feedback` comme source de vérité pour le produit. Voir [ARCH.md](ARCH.md) et [DEVELOPMENT.md](DEVELOPMENT.md) (import Promptfoo → dataset LangSmith, expérience `evaluate` dans `scripts/langsmith-eval/`).
