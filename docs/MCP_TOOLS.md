...
| `create_activity_type` | `code` (string), `label` (string), `dailyRateCents` (int), `description` (string, optional) | `activity_type` | Creates an activity type. Code is stored uppercase. Use when the user asks to add a nature or rate (e.g. "add CONSULT at 800 €/day" → dailyRateCents 80000; non-billable training at 0 €/day → dailyRateCents 0). |
...
| `create_time_log` | `projectId` (UUID or name), `durationMinutes` (int), `note` (optional), `loggedAt` (ISO-8601, optional), `billable` (boolean, optional), `activityTypeCode` (string, optional) | `time_log` | Records a time entry. If the user mentions an activity nature (dev, IA, marketing), pass the matching code from list_activity_types. Returns the created time_log with id, projectId, projectName, durationMinutes, note, billable, loggedAt, and when set activityTypeCode, activityTypeLabel, dailyRateCents, and optional projectCardColorIndex when the project has a user-chosen card color. |
| `get_recent_logs` | `limit` (int, optional) | `time_logs[]` (each with id, projectId, projectName, durationMinutes, note, billable, loggedAt, and when set activityTypeCode, activityTypeLabel, dailyRateCents, projectCardColorIndex) | Returns the most recent time logs (default 20, max 50). `projectCardColorIndex` is present when the project has a user-chosen card color. |
| `get_time_logs_for_period` | `start`, `end` (ISO-8601), `projectId` (optional) | `time_logs[]` | Returns logs in the date range (each entry includes billable and, when set, activity type fields and optional projectCardColorIndex). |
| `search_time_logs` | `query` (string), `limit` (int, optional) | `time_logs[]` | Search time logs by keyword in note or project name (case-insensitive). Use when the user asks to find entries containing a word or phrase. Then call propose_entries to display. Do NOT use for listing by date—use get_time_logs_for_period or get_recent_logs. Entries may include projectCardColorIndex when set on the project. |
...
| `update_time_log` | `id` (UUID), `durationMinutes`, `note`, `loggedAt`, `projectId`, `billable`, `activityTypeCode` (string or omit; send empty to clear) (all optional except id) | `time_log` | Updates an existing time log. Returns the updated time_log with activity type fields when set and optional projectCardColorIndex when set on the project. |
