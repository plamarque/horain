 ...
 *
 * Optional: **Promptfoo scored (LLM-as-judge)** — same Mistral rubrics as `promptfoo/tests/scored/*.yaml`,
 * matched by `metadata.description` on each example. See `promptfoo_llm_applied` / `promptfoo_llm_score` columns.
 *
 * Prerequisites:
 *   - Backend reachable (default http://localhost:8080) with seed if needed
 *   - LANGCHAIN_API_KEY (or LANGSMITH_API_KEY) — often in backend/.env
 *   - LANGSMITH_DATASET_ID = UUID of the dataset
 *   - For LLM judge: PROMPTFOO_JUDGE_MISTRAL_API_KEY (promptfoo/.env or env); optional PROMPTFOO_JUDGE_MODEL
 *
 * Env loading: backend/.env then promptfoo/.env unless HORAIN_SKIP_BACKEND_ENV=1
 *
 * Flags:
 *   --with-scored-judge     force Mistral judge (exit if API key missing)
 *   --no-scored-judge       disable Mistral judge even if key is set
 *
...
const DATASET_ID = process.env.LANGSMITH_DATASET_ID?.trim()
...
/**
 * Target: same contract as Promptfoo horain-api provider output shape (message, data, toolCalls).
 * @param {Record<string, unknown>} inputs dataset example inputs
 */
...
async function horainTarget(inputs) {
  ...
  if (typeof message !== 'string' || !message.trim()) {
    throw new Error('Dataset inputs must include user_message (see import-promptfoo-to-langsmith.mjs)')
  }
...
/**
 * Code-style evaluator: no HTTP error and a non-empty assistant message string.
 */
...
/**
 * Uses dataset metadata from Promptfoo import (description, promptfoo_file).
 */
...
/**
 * Mistral LLM-as-judge aligned with Promptfoo `llm-rubric` + `rubricPrompt` in scored YAML.
 * Returns two keys: promptfoo_llm_applied (0|1), promptfoo_llm_score (0..1, or 0 when not applied / error).
 */
...
async function main() {
  ...
  if (!DATASET_ID) {
    console.error(
      'Set LANGSMITH_DATASET_ID to your dataset UUID (create via import-promptfoo-to-langsmith.mjs or LangSmith UI).'
    )
    process.exit(1)
  }
  if (!client.apiKey) {
  ...
  console.error(
    'Tip: start the backend before running (e.g. from backend/: mvn spring-boot:run). If fetch fails, rows get error outputs but the script should still exit after "Done."'
  )
